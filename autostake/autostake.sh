...
ADDR=$(${service} keys show $wallet -a --bech acc ${keyring})
VAL=$(${service} keys show $wallet -a --bech val ${keyring})
PWD='PWD' # password
...
for (( ;; )); do
        ...
        echo -e "${PWD}\ny\n" | ${service} tx distribution withdraw-rewards ${VAL} --chain-id ${chain} --from $wallet  --commission ${keyring} ${gasfees} -y
for (( timer=20; timer>0; timer-- ))
        do
                printf "* sleep for ${RED_COLOR}%02d${WITHOUT_COLOR} sec\r" $timer
                sleep 1
        done
BAL=$(${service} q bank balances ${ADDR} -o json | jq -r '.balances | .[].amount')
...
echo -e "BALANCE: ${GREEN_COLOR}${BAL}${WITHOU_COLOR} ${denom}\n"
        echo -e "Stake ALL\n"
if (( BAL > 1000 )); then
            echo -e "${PWD}\n${PWD}\n" | ${service} tx staking delegate ${VAL} ${BAL}${denom} --from $wallet --chain-id ${chain} ${keyring} ${gasfees} -y
        else
